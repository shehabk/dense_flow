...
void calcDenseFlowGPUFrame(string from_file, string to_file, int bound, int type, int step, int dev_id,
    ...
    while(true){
        ...
        if (!initialized){
           capture_frame = imread(from_file,CV_LOAD_IMAGE_COLOR );
           if (capture_frame.empty()) return; // read frames until end
            ...
            capture_frame = imread(to_file,CV_LOAD_IMAGE_COLOR );
            if (!do_resize)
