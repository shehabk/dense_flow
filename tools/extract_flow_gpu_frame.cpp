...
int main(int argc, char** argv){
	...
	const char* keys =
		{
			"{ fr fromFile    | from.jpg | filename of fromframe }"
			"{ to toFile      | to.jpg | filename of toframe }"
	...
	std::string fromFile = cmd.get<std::string>("fromFile");
	std::string toFile = cmd.get<std::string>("toFile");
	...
	// cout<<xFlowFile<<endl;
	// cout<<device_id<<endl;
	calcDenseFlowGPUFrame(fromFile, toFile, bound, type, step, device_id,
					 out_vec_x, out_vec_y, out_vec_img, new_width, new_height);

	if (output_style == "dir") {
		writeImage(out_vec_x, xFlowFile);
		writeImage(out_vec_y, yFlowFile);
		writeImage(out_vec_img, imgFile);
	}else{
